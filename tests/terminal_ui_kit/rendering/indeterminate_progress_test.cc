...
TEST(IndeterminateProgress, AdvancesSegmentAfterFrameDuration) {
  ProgressBarOptions options;
  options.width = 4;
  ftxui::Component progress = IndeterminateProgress(
      default_dark_theme(), options, 2, std::chrono::milliseconds(50));
...
TEST(IndeterminateProgress, WrapsSegmentAtRightEdge) {
  ...
  options.width = 3;
  ftxui::Component progress = IndeterminateProgress(
      default_dark_theme(), options, 1, std::chrono::milliseconds(10));
