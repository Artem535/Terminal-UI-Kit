...
TEST(EditorRecallHistory, EmptyByDefault) {
  CommandHistory history;
  EXPECT_TRUE(history.empty());
  EXPECT_EQ(history.size(), 0U);
}

TEST(EditorRecallHistory, AddStoresOwnedEntriesNewestLast) {
  CommandHistory history;
  history.add("one");
  history.add("two");
  history.add("three");
...
TEST(EditorRecallHistory, SkipsEmptyEntries) {
  CommandHistory history;
  history.add("");
  history.add("  ");
  history.add("real");
...
TEST(EditorRecallHistory, DeduplicatesConsecutiveEntries) {
  CommandHistory history;
  history.add("same");
  history.add("same");
  history.add("other");
...
TEST(EditorRecallHistory, RingBufferDropsOldestBeyondMax) {
...
TEST(EditorRecallHistory, PreviousWalksBackThenNextWalksForward) {
  CommandHistory history;
  history.add("a");
  history.add("b");
  history.add("c");
...
TEST(EditorRecallHistory, PreviousEmptyReturnsFalse) {
  CommandHistory history;
  std::string out;
  EXPECT_FALSE(history.previous(out));
}

TEST(EditorRecallHistory, NextWhenNotNavigatingReturnsFalse) {
  CommandHistory history;
  history.add("a");
  std::string out;
...
TEST(EditorRecallHistory, AddExitsNavigationClearsDraft) {
  CommandHistory history;
  history.add("a");
  history.add("b");
  history.set_draft("draft");
...
TEST(EditorRecallHistory, AddExitsNavigationEvenForBlankOrDuplicate) {
  CommandHistory history;
  history.add("a");
  history.set_draft("draft");
  std::string out;
...
TEST(EditorRecallHistory, EndNavigationClearsDraft) {
  CommandHistory history;
  history.add("a");
  history.set_draft("draft");
  std::string out;
...
TEST(EditorRecallHistory, ClearEmptiesHistory) {
  CommandHistory history;
  history.add("a");
  history.add("b");
  history.clear();
