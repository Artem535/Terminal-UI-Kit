 ...
 private:
  ...
  ftxui::Element render_display_line(std::size_t index, int /*width*/) {
    ...
    if (options_.show_line_numbers) {
      if (line.sub_line == 0) {
        std::string num =
            pad_left_to_width(std::to_string(line.logical_line + 1), options_.gutter_width);
        parts.push_back(ftxui::text(num) | ftxui::color(ftxui::Color::GrayDark));
        parts.push_back(ftxui::text(" "));
      } else {
        // Continuation line of a wrapped logical line: indent to mirror the
        // old fixed gutter (gutter_width digits + number separator + one
        // padding column), preserving the existing continuation offset.
        parts.push_back(ftxui::text(std::string(options_.gutter_width + 2, ' ')));
      }
    }
